...
class LoanBookLogic:
    @staticmethod
    def loan_book(db, loan_book_request: LoanBookCreate, created_by: int):
        ...
        quantity = BookInventoryRepository.get_quantity_added(db, new_loan_book_request.book_id) or 0
        
        book_available = quantity - LoanBookRepository.get_not_returned_loans(db, new_loan_book_request.book_id)
...
class ReturnLoanLogic:
    ...
    def return_loan(db, loan_id):
        loan = LoanBookRepository.get_loaned_book_by_id(db, loan_id)
