...
class LoanService:
    @staticmethod
    async def loan_book(
        db: AsyncSession, current_user_id: int, loan_request: LoanBase
    ) -> Loan:
        ...
        book_available = await InventoryRepository.get_available_inventories(
            db, loan_request.book_id
        )
        ...
        try:
            ...
            inventory = await InventoryRepository.get_inventory_by_id(
                db, new_loan.inventory_id
            )
    ...
    async def get_loans(
        ...
        loans, total = await LoanRepository.get_loans(
...
class LoanServicePublic:
    ...
    async def loan_book_me(
        db: AsyncSession, loan_request: CreateLoanPublic, user_id: int
    ) -> Loan:
        book = await BookRepository.get_book_by_id(db, loan_request.book_id)
        ensure_exists(book, BookNotFoundError(HTTP404.BOOK))
